...
# %%
batch_size = 1
trajs, qpos_arrays = sample_n_traj(mj_model, mj_data, n_steps=n_steps, length_scale=1.0, qpos_scale=0.2, n_traj=batch_size, PLOT=True)
trajs # bug only one values!
...
v = np.zeros(6*n_steps)
a = torch.arange(10).reshape(2,5)
torch.maximum(a, torch.zeros_like(a))

# %%
torch.max(torch.arange(10).reshape(2,5),dim=1)